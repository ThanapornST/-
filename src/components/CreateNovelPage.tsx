...
import { PenLine, BookOpen, FolderOpen, FileText, BookMarked, LogOut, Plus, Upload } from 'lucide-react';
...
interface Project {
  ...
  description: string;
  coverImage?: string;
  uploadedText?: string; // Add uploadedText property
}
...
const CreateNovelPage = () => {
  ...
  const handleUploadNovel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        const novelText = event.target?.result as string;
        // Create a new project with the uploaded novel text
        const newProject: Project = {
          id: Date.now().toString(),
          title: file.name, // Use the file name as the title
          description: 'Uploaded Novel',
          uploadedText: novelText, // Store the novel text
        };
        setProjects([newProject, ...projects]);
      };
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <input
              ...
              ref={fileInputRef}
            />
            <label htmlFor="uploadNovel" className="mt-4 inline-block bg-[#0f0f0f] px-4 py-2 rounded-lg cursor-pointer hover:opacity-80 transition-opacity">
